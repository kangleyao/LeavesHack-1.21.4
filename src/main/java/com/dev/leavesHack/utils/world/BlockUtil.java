...
public class BlockUtil {
    ...
    public static Direction getClickSideStrict(BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            double distanceSq = mc.player.getEyePos().squaredDistanceTo(pos.offset(direction).toCenterPos());
    ...
    public static boolean noEntityBlockCrystal(BlockPos pos, boolean ignoreCrystal, boolean ignoreItem) {
        for (Entity entity : getEntities(new Box(pos))) {
            ...
            if (ignoreCrystal && entity instanceof EndCrystalEntity && mc.player.getEyePos().distanceTo(getClosestPoint(entity)) <= AutoCity.INSTANCE.range.get()) continue;
            return false;
        }
    ...
    public static boolean canPlace(BlockPos pos, boolean ignoreSneak) {
        if (getPlaceSide(pos, null, ignoreSneak) == null) return false;
    ...
    public static boolean hasEntity(BlockPos pos, boolean ignoreCrystal) {
        for (Entity entity : getEntities(new Box(pos))) {
            if (!entity.isAlive()) continue;
            if (entity instanceof ItemEntity || entity instanceof ExperienceOrbEntity || entity instanceof ExperienceBottleEntity || entity instanceof ArrowEntity) continue;
            if (ignoreCrystal && entity instanceof EndCrystalEntity) continue;
            return true;
        }
    ...
    public static List<Entity> getEntities(Box box) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (entity != null && entity.getBoundingBox().intersects(box)) list.add(entity);
        }
        ...
        for (double x = pos.getX() - range; x < pos.getX() + range; x++) {
            for (double z = pos.getZ() - range; z < pos.getZ() + range; z++) {
                for (double y = pos.getY() - range; y < pos.getY() + range; y++) {
                    ...
                    if (!list.contains(curPos)) list.add(curPos);
                }
            }
        }
    ...
    public static List<EndCrystalEntity> getEndCrystals(Box box) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof EndCrystalEntity crystal && crystal.getBoundingBox().intersects(box)) list.add(crystal);
        }
    ...
    public static Direction getPlaceSide(BlockPos pos, Predicate<Direction> directionPredicate) {
        ...
        for (Direction direction : Direction.values()) {
            if (directionPredicate != null && !directionPredicate.test(direction)) continue;

            BlockPos offset = pos.offset(direction);
            if (canClick(offset) && !mc.world.getBlockState(offset).isReplaceable()) {
                if (!isGrimDirection(offset, direction.getOpposite())) continue;

                double vecDistance = mc.player.getEyePos().squaredDistanceTo(pos.toCenterPos().add(direction.getVector().getX() * 0.5, direction.getVector().getY() * 0.5, direction.getVector().getZ() * 0.5));
                if (side == null || vecDistance < distance) {
                    side = direction;
    ...
    public static Direction getPlaceSide(BlockPos pos, Predicate<Direction> directionPredicate, boolean ignoreSneak) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            BlockPos offset = pos.offset(direction);
            if (canClick(offset, ignoreSneak) && !mc.world.getBlockState(offset).isReplaceable()) {
                if (!isGrimDirection(offset, direction.getOpposite())) continue;
    ...
    public static ArrayList<Direction> getPlaceSides(BlockPos pos, Predicate<Direction> directionPredicate) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (canClick(neighbor) && !neighborState.isReplaceable() && isGrimDirection(neighbor, direction.getOpposite())) sides.add(direction);
        }
    ...
    public static ArrayList<Direction> getPlaceSides(BlockPos pos, Predicate<Direction> directionPredicate, boolean ignoreSneak) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            BlockPos neighbor = pos.offset(direction);
            BlockState neighborState = mc.world.getBlockState(neighbor);
            if (canClick(neighbor, ignoreSneak) && !neighborState.isReplaceable() && isGrimDirection(neighbor, direction.getOpposite())) sides.add(direction);
        }
    ...
    public static Direction getClickSide(BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            double distance = MathHelper.sqrt((float) mc.player.getEyePos().squaredDistanceTo(pos.offset(direction).toCenterPos()));
    ...
    private static Box getCombinedBox(BlockPos pos, World world) {
    ...
    public static void clickBlock(BlockPos pos, Direction side, boolean rotate) {
        ...
        if (usePacketPlace()) sendSequencedPacket(id -> new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, result, id));
        else mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, result);

        if (rotate) Rotation.snapBack();
    }

    public static boolean needSneak(Block block) {
    ...
    private static boolean usePacketPlace() {
        return GlobalSetting.INSTANCE != null && GlobalSetting.INSTANCE.packetPlace.get();
    }

    private static void sendSequencedPacket(SequencedPacketCreator packetCreator) {
