...
public class FireworkElytraFly extends Module {
    ...
    public void onTick(TickEvent.Pre event){
        ...
        boolean wearingElytra = mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem() == Items.ELYTRA && isElytraUsable(mc.player.getEquippedStack(EquipmentSlot.CHEST));
        if (!wearingElytra || mc.player.isOnGround()) {
            // mc.player.stopFallFlying(); // Removed in 1.21.4
            return;
        }
        if (!mc.player.isOnGround() && mc.options.jumpKey.isPressed()) {
            sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
            // mc.player.startFallFlying(); // Removed in 1.21.4
        }
