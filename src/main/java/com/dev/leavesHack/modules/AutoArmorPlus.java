...
public class AutoArmorPlus extends Module {
    ...
    public void onTick(TickEvent.Pre event){
        ...
        for (int s = 0; s < 36; s++) {
            ...
            EquippableComponent equippable = mc.player.getInventory().getStack(s).get(DataComponentTypes.EQUIPPABLE);
            EquipmentSlot slot = (mc.player.getInventory().getStack(s).getItem() == Items.ELYTRA ? EquipmentSlot.CHEST : (equippable != null ? equippable.slot() : null));
            for (Map.Entry<EquipmentSlot, int[]> e : armorMap.entrySet()) {
                ...
                if (autoElytra.get() && fireworkElytraFly.isActive() && e.getKey() == EquipmentSlot.CHEST) {
                    if (!mc.player.getInventory().getStack(38).isEmpty() && mc.player.getInventory().getStack(38).isOf(Items.ELYTRA) && isElytraUsable(mc.player.getInventory().getStack(38))) {
                        continue;
                    }
                    if (e.getValue()[2] != -1 && !mc.player.getInventory().getStack(e.getValue()[2]).isEmpty() && mc.player.getInventory().getStack(e.getValue()[2]).isOf(Items.ELYTRA) && isElytraUsable(mc.player.getInventory().getStack(e.getValue()[2]))) {
                        continue;
                    }
                    if (!mc.player.getInventory().getStack(s).isEmpty() && mc.player.getInventory().getStack(s).isOf(Items.ELYTRA) && isElytraUsable(mc.player.getInventory().getStack(s))) {
                        e.getValue()[2] = s;
                    }
                    continue;
                }
    ...
    private int getProtection(ItemStack is) {
        if (is.getItem() instanceof ArmorItem || is.getItem() == Items.ELYTRA) {
            ...
            if (is.hasEnchantments()) {
                if (ignoreBinding.get() && InventoryUtil.getEnchantmentLevel(is, Enchantments.BINDING_CURSE) > 0) return -1;
                prot += InventoryUtil.getEnchantmentLevel(is, Enchantments.PROTECTION);
            }
            return (is.getItem() instanceof ArmorItem ? 1 : 0) + prot;
        } else if (!is.isEmpty()) {
            return 0;
        }
        return -1;
